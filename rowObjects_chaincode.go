...
func (t *FabricChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "updateObject" { //change owner of a specific rowObject
		return t.updateObject(stub, args)
	} else if function == "updateObjectsBasedOnColor" { //transfer all rowObjects of a certain color
		return t.updateObjectsBasedOnColor(stub, args)
	} else if function == "delete" { //delete a rowObject
		return t.delete(stub, args)
	} else if function == "getObject" { //read a rowObject
		return t.getObject(stub, args)
	} else if function == "queryObjectsByOwner" { //find rowObjects for owner X using rich query
		return t.queryObjectsByOwner(stub, args)
	} else if function == "queryObjects" { //find rowObjects based on an ad hoc rich query
		return t.queryObjects(stub, args)
	} else if function == "getHistoryForObject" { //get history of values for a rowObject
		return t.getHistoryForObject(stub, args)
	} else if function == "getObjectsByRange" { //get rowObjects based on range query
		return t.getObjectsByRange(stub, args)
	} else if function == "getObjectsByRangeWithPagination" {
		return t.getObjectsByRangeWithPagination(stub, args)
...
func (t *FabricChaincode) addNewObject(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	rowObject := &rowObject{objectType, rowObjectKey, color, size, owner}
	rowObjectJSONasBytes, err := json.Marshal(rowObject)
	if err != nil {
		return shim.Error(err.Error())
	}
	//Alternatively, build the rowObject json string manually if you don't want to use struct marshalling
	//marbleJSONasString := `{"docType":"Marble",  "name": "` + rowObjectKey + `", "color": "` + color + `", "size": ` + strconv.Itoa(size) + `, "owner": "` + owner + `"}`
	...
	//  ==== Index the rowObject to enable color-based range queries, e.g. return all blue rowObjects ====
...
func (t *FabricChaincode) updateObject(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	objKey := args[0]
	newProp04 := strings.ToLower(args[1])
	fmt.Println("- start updateObject ", objKey, newProp04)

	marbleAsBytes, err := stub.GetState(objKey)
	if err != nil {
		return shim.Error("Failed to get rowObject:" + err.Error())
	...
	marbleToTransfer := rowObject{}
	err = json.Unmarshal(marbleAsBytes, &marbleToTransfer) //unmarshal it aka JSON.parse()
	if err != nil {
		return shim.Error(err.Error())
	}
	marbleToTransfer.Prop04 = newProp04 //change the owner

	marbleJSONasBytes, _ := json.Marshal(marbleToTransfer)
	err = stub.PutState(objKey, marbleJSONasBytes) //rewrite the rowObject
	if err != nil {
		return shim.Error(err.Error())
	}
...
// ===========================================================================================
func (t *FabricChaincode) getObjectsByRange(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
// updateObjectsBasedOnColor will transfer rowObjects of a given color to a certain new owner.
...
// Therefore, range queries are a safe option for performing update transactions based on query results.
// ===========================================================================================
func (t *FabricChaincode) updateObjectsBasedOnColor(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	fmt.Println("- start updateObjectsBasedOnColor ", color, newOwner)
	...
	for i = 0; coloredMarbleResultsIterator.HasNext(); i++ {
		// Note that we don't get the value (2nd return variable), we'll just get the rowObject name from the composite key
		...
		fmt.Printf("- found a rowObject from index:%s color:%s name:%s\n", objectType, returnedColor, returnedMarbleName)
	...
	responsePayload := fmt.Sprintf("Transferred %d %s rowObjects to %s", i, color, newOwner)
	fmt.Println("- end updateObjectsBasedOnColor: " + responsePayload)
...
// =========================================================================================
func (t *FabricChaincode) queryObjectsByOwner(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
// Only available on state databases that support rich query (e.g. CouchDB)
// =========================================================================================
func (t *FabricChaincode) queryObjects(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
// ===========================================================================================
func (t *FabricChaincode) getObjectsByRangeWithPagination(stub shim.ChaincodeStubInterface, args []string) pb.Response {
